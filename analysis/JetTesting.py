...
processList = {
    ...
    # signal
    f'wzp6_ee_bbH_HZZ_ecm240': {'fraction':1},
}


sigProcs = ["wzp6_ee_qqH_ecm240", "wzp6_ee_mumuH_ecm240", "wzp6_ee_bbH_ecm365", "wzp6_ee_bbH_HZZ_ecm240"]
...
def build_graph(df, dataset):
    ...
    #Build Jets: 
    df= df.Define("RP_px", "FCCAnalyses::ReconstructedParticle::get_px(RPS_no_mu)")
    df= df.Define("RP_py", "FCCAnalyses::ReconstructedParticle::get_py(RPS_no_mu)")
    df= df.Define("RP_pz", "FCCAnalyses::ReconstructedParticle::get_pz(RPS_no_mu)")
    df = df.Define("RP_e", "FCCAnalyses::ReconstructedParticle::get_e(RPS_no_mu)")
    df = df.Define("pseudo_jets", "FCCAnalyses::JetClusteringUtils::set_pseudoJets(RP_px, RP_py, RP_pz, RP_e)")


    #Do Jet Clustering: 
    df = df.Define("clustered_jets", "JetClustering::clustering_ee_kt(3,2,0,10)(pseudo_jets)")
    ...
    df = df.Define("jets_truth", "FCCAnalyses::JHUfunctions::jetTruthFinder(jetconstituents, RPS_no_mu, Particle, MCRecoAssociations1)") # returns best-matched PDG ID of the jets
    df = df.Define("Z_mass", "(jets_tlv[0]+jets_tlv[1]).M()")
    df = df.Define("Z_pt","(jets_tlv[0]+jets_tlv[1]).Pt()" )
    df = df.Define("Z_tlv", "(jets_tlv[0]+jets_tlv[1])")
