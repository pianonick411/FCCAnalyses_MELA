...
outputDir   = "/eos/user/n/nipinto/FCCAnalyses_MELA/analysis/FCCAnalysisOut/leptonicAngles/test"
...
def build_graph(df, dataset):
    ...
    df = df.Define("ghz4PairOne", 'FCCAnalyses::JHUfunctions::makeCouplingPairs("ghz4", 1.0, 0)')
    ...
    df = df.Define("BSMVecOne", 'FCCAnalyses::JHUfunctions::createCouplingVector({ghz4PairOne})')
   ...
   # df = df.Define("positronMC", "FCCAnalyses::MCParticle::sel_byIndex(positron_decays[0], Particle)")
        ...
        df = df.Define("BSMWeightsOne",'FCCAnalyses::JHUfunctions::Weights(LHEDaughterId, higgsMCTLV, LHEAssociatedParticleId, negmuTLV, posmuTLV, BSMVecOne)' )
        # df = df.Define("5050InterferenceWeights", '')

        df = df.Define("MC_MELA_Angles", "FCCAnalyses::JHUfunctions::MELAAngles(electronTLV, 11, positronTLV, -11, negmuTLV, 13, posmuTLV, -13)")

        df = df.Define("MC_cos_1", "MC_MELA_Angles[1]")
        df = df.Define("MC_cos_2", "MC_MELA_Angles[2]")
        df = df.Define("MC_phi", "MC_MELA_Angles[3]")
        df = df.Define("MC_phi1", "MC_MELA_Angles[4]")
        df = df.Define("MC_mVstar", "MC_MELA_Angles[5]")
        df = df.Define("MC_mV", "MC_MELA_Angles[6]")
   ...
   # df = df.Define("whatever", "FCCAnalyses::JHUfunctions::yeah(1)")
    ...
    df = df.Define("RecoNegInterferenceWeights", 'RecoNegMixtureWeights - RecoSMWeights - RecoBSMWeights')
    ...
    df = df.Define("D_CP_Neg", 'RecoNegInterferenceWeights/(2*sqrt(RecoSMWeights*RecoBSMWeights))')
    ...
    df = df.Define("BSMWeightsOneNORM", 'BSMWeightsOne/SMWeights')
   ...
   #results.append(df.Histo1D(("parent1pdg", "", *bins_count), "parent1pdg", "nominal_weight"))
    ...
    # sig_branchlist.push_back('BSMWeightsOneNORM')
    ...
    #df.Snapshot("tree", f"{flavor}Snapshots/{dataset}.root", branchlist)
    #df.Snapshot("tree", f"mumuSnapshots/SignalFixed.root", sig_branchlist)
